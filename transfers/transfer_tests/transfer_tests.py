...
def test_transfer_item_actions(client, test_company_token_fixture):
    ...
    response = client.post(reverse('product-transfer-item-attach', args=[item_pk]), {"transfer_id": 1}, content_type='application/json', HTTP_AUTHORIZATION=f'Bearer {test_company_token_fixture}')
    assert response.status_code == 201
...
def test_stock_transfer_workflow_actions(client, test_company_token_fixture):
    ...
    pk = 1
    # 1. Attach/Detach
    response = client.post(reverse('product-transfer-attach', args=[pk]), {"item_id": 1}, content_type='application/json', HTTP_AUTHORIZATION=f'Bearer {test_company_token_fixture}')
    assert response.status_code == 201
