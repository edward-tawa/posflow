...
class CustomerRefundView(APIView):
    ...
    def post(self, request, customer_id, *args, **kwargs):
        ...
        try:
            CustomerCreditService.record_customer_refund(customer, amount)

            return Response(
                {"message": f"Refund of {amount} recorded for customer {customer.first_name}."},
                status=status.HTTP_201_CREATED
            )

        except Exception as e:
            logger.error(f"Error recording customer refund: {e}")
            return Response(
                {"error": "An error occurred while recording the refund."},
