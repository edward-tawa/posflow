...
class SalesAccountSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        if validated_data['account'].company != company:
            logger.error(
                f"{actor} attempted to create a SalesAccount for a different company."
            )
            raise serializers.ValidationError(
                "You cannot create a SalesAccount for a different company's data."
            )
        ...
        try:
            ...
            logger.error(
                f"{actor} failed to create SalesAccount: {str(e)}"
            )
            raise serializers.ValidationError("Failed to create SalesAccount.") from e
    ...
    def update(self, instance, validated_data):
        ...
        if instance.account.company != company:
            logger.error(
                f"{actor} attempted to update a SalesAccount for a different company."
            )
            raise serializers.ValidationError(
                "You cannot update a SalesAccount for a different company's data."
            )
        ...
        try:
            ...
            instance.save()
            logger.info(
                f"{actor} updated SalesAccount for '{instance.account.name}' (ID: {instance.id})."
            )
            return instance
        except Exception as e:
            logger.error(
                f"{actor} failed to update SalesAccount '{instance.account.name}' (ID: {instance.id}): {str(e)}"
            )
            raise serializers.ValidationError("Failed to update SalesAccount.") from e
