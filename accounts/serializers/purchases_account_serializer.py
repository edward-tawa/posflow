...
class PurchasesAccountSerializer(serializers.ModelSerializer):
    ...
    recipient_person = serializers.PrimaryKeyRelatedField(
    ...
    def create(self, validated_data):
        ...
        if validated_data['account'].company != company:
            logger.error(
                f"{actor} attempted to create a PurchasesAccount for a different company."
            )
            raise serializers.ValidationError(
                "You cannot create a PurchasesAccount for a different company's data."
            )

        try:
            purchases_account = PurchasesAccount.objects.create(**validated_data)
            logger.info(
                f"{actor} created PurchasesAccount for '{purchases_account.account.name}' (ID: {purchases_account.id})."
            )
            return purchases_account
        except Exception as e:
            logger.error(
                f"{actor} failed to create PurchasesAccount: {str(e)}"
            )
            raise serializers.ValidationError("Failed to create PurchasesAccount.") from e
    ...
    def update(self, instance, validated_data):
        ...
        if instance.account.company != company:
            logger.error(
                f"{actor} attempted to update a PurchasesAccount for a different company."
            )
            raise serializers.ValidationError(
                "You cannot update a PurchasesAccount for a different company's data."
            )
        ...
        try:
            ...
            instance.save()
            logger.info(
                f"{actor} updated PurchasesAccount for '{instance.account.name}' (ID: {instance.id})."
            )
            return instance
        except Exception as e:
            logger.error(
                f"{actor} failed to update PurchasesAccount '{instance.account.name}' (ID: {instance.id}): {str(e)}"
            )
            raise serializers.ValidationError("Failed to update PurchasesAccount.") from e
