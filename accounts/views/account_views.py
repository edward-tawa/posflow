...
class AccountViewSet(ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        logger.bind(account_number=account.number).success(
            f"Account '{account.name}' (Number: {account.number}) created by {actor} in company '{getattr(company, 'name', 'Unknown')}'."
        )
    ...
    def perform_update(self, serializer):
        ...
        actor = getattr(company, 'name', None) or getattr(user, 'username', 'Unknown')
        logger.bind(account_number=account.number).info(
            f"Account '{account.name}' (Number: {account.number}) updated by {actor}."
        )
    ...
    def perform_destroy(self, instance):
        ...
        company = getattr(user, 'company', None) or (user if isinstance(user, Company) else None)
        actor = getattr(company, 'name', None) or getattr(user, 'username', 'Unknown')
        logger.bind(account_number=instance.number).warning(
            f"Account '{instance.name}' (Number: {instance.number}) deleted by {actor}."
        )
        instance.delete()