...
from payments.serializers.sales_payment_serializer import SalesPaymentSerializer
...
class SalesPaymentViewSet(ModelViewSet):
    """
    ViewSet for managing SalesPayments.
    ...
    def get_queryset(self):
        try:
            return get_company_queryset(self.request, SalesPayment).select_related(
                'company', 'branch', 'sales_order', 'sales_invoice', 'payment', 'received_by'
            )
        except Exception as e:
            logger.error(f"Error fetching SalesPayment queryset: {e}")
    ...
    def perform_create(self, serializer):
        ...
        sales_payment = serializer.save()
        actor = getattr(company, 'name', None) or getattr(user, 'username', 'Unknown')
        logger.success(
            f"SalesPayment '{sales_payment.id}' created by '{actor}' "
            f"for company '{sales_payment.company.name}'."
        )
    ...
    def perform_update(self, serializer):
        ...
        actor = getattr(company, 'name', None) or getattr(user, 'username', 'Unknown')
        logger.info(
            f"SalesPayment '{sales_payment.id}' updated by '{actor}' "
            f"for company '{sales_payment.company.name}'."
        )
    ...
    def perform_destroy(self, instance):
        ...
        actor = getattr(company, 'name', None) or getattr(user, 'username', 'Unknown')
        logger.warning(
            f"SalesPayment '{payment_id}' deleted by '{actor}' "
