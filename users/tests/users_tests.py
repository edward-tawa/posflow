...
@pytest.mark.django_db
def test_user_url_create(client, test_company_token_fixture):
    url = reverse('user-list')
    ...
    response = client.post(url_activate, data, content_type='application/json', HTTP_AUTHORIZATION=f'Bearer {test_user_token}')
    logger.info(response.json())
    assert response.status_code == 200

    # 2. Deactivate Bulk
    url_deactivate = reverse('user-deactivate-bulk')
    response = client.post(url_deactivate, data, content_type='application/json', HTTP_AUTHORIZATION=f'Bearer {test_user_token}')
    logger.info(response.json())
    assert response.status_code == 200

    # 3. Delete Bulk
    url_delete = reverse('user-delete-bulk')
    response = client.post(url_delete, data, content_type='application/json', HTTP_AUTHORIZATION=f'Bearer {test_user_token}')
    logger.info(response.json())
    assert response.status_code == 200

    # 4. Assign Role Bulk
    url_assign = reverse('user-assign-role-bulk')
    data_role = {"user_ids": [1, 2], "role": "manager"}
    response = client.post(url_assign, data_role, content_type='application/json', HTTP_AUTHORIZATION=f'Bearer {test_user_token}')
    logger.info(response.json())
    assert response.status_code == 200

@pytest.mark.django_db
def test_granular_user_actions(client, test_company_token_fixture):
    """
    Test specific actions on individual users.
    """
    #test create user to update
    url = reverse('user-list')
    ...
    response = client.put(reverse('user-update', args=[user_id]), {"username": "updated"}, content_type='application/json', HTTP_AUTHORIZATION=f'Bearer {test_company_token_fixture}')
    logger.info(response.json())
    assert response.status_code == 200

    response = client.delete(reverse('user-delete', args=[user_id]), HTTP_AUTHORIZATION=f'Bearer {test_company_token_fixture}')
    logger.info(response.json())
    assert response.status_code == 200

    # 2. Status Actions
    response = client.post(reverse('user-activate', args=[user_id]), {}, content_type='application/json', HTTP_AUTHORIZATION=f'Bearer {test_company_token_fixture}')
    logger.info(response.json())
    assert response.status_code == 200

    response = client.post(reverse('user-deactivate', args=[user_id]), {}, content_type='application/json', HTTP_AUTHORIZATION=f'Bearer {test_company_token_fixture}')
    logger.info(response.json())
    assert response.status_code == 200

    # 3. Role Management
    response = client.post(reverse('user-assign-role', args=[user_id]), {"role": "MANAGER"}, content_type='application/json', HTTP_AUTHORIZATION=f'Bearer {test_company_token_fixture}')
    logger.info(response.json())
    assert response.status_code == 200

    response = client.post(reverse('user-remove-role', args=[user_id]), {}, content_type='application/json', HTTP_AUTHORIZATION=f'Bearer {test_company_token_fixture}')
    logger.info(response.json())
    assert response.status_code == 200

    # 4. Password Reset (Individual & Bulk)
    response = client.post(reverse('user-reset-password', args=[user_id]), {"new_password": "123"}, content_type='application/json', HTTP_AUTHORIZATION=f'Bearer {test_company_token_fixture}')
    logger.info(response.json())
    assert response.status_code == 200

    response = client.post(reverse('user-reset-password-bulk'), {"ids": [1], "new_password": "123"}, content_type='application/json', HTTP_AUTHORIZATION=f'Bearer {test_company_token_fixture}')
    logger.info(response.json())
    assert response.status_code == 200
