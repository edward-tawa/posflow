...
class MarkTransactionPending(APIView):
    ...
    def post(self, request, transaction_id):
        """
        Marks a transaction as pending and logs the event.
        """
        ...
        try:
            ...
            TransactionService.mark_transaction_pending(transaction)
            actor = getattr(company, 'name', None) or getattr(user, 'username', 'Unknown')
            logger.info(f'Transaction {transaction.transaction_number} marked as pending by {actor} for company {company.name}')
            return Response({"status":"Success", "message":f"Transaction '{transaction.transaction_number}' marked as pending successfully."}, status=status.HTTP_200_OK)
            ...
            logger.exception(f"Error marking transaction {transaction_id} as pending")
            return Response({"status":"Failure", "message":"An error occurred while marking the transaction as pending."}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
